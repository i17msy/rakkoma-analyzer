...
# ── クラウド連携: Cloudflare R2（死活監視ハートビート＋DBバックアップ・S3互換）──
# 認証は環境変数で注入（run_container -e）。未設定なら全て no-op（ローカルバックアップのみ動く）:
#   R2_ACCOUNT_ID / R2_ACCESS_KEY_ID / R2_SECRET_ACCESS_KEY  … 必須
#   R2_BUCKET（任意・既定 rakkoma-analyzer）/ R2_ENDPOINT（任意・通常 account_id から自動）
R2_BUCKET_DEFAULT    = "rakkoma-analyzer"
R2_HEARTBEAT_KEY     = "heartbeat/observer.json"   # observer が毎ポーリングで更新（生死信号）
R2_BACKUP_PREFIX     = "backups/"                  # 日次DBバックアップの置き場
BACKUPS_DIR          = DATA_DIR / "backups"        # ローカル日次スナップショット
BACKUP_RETENTION     = 14                          # ローカル/R2 とも直近N日分を保持
HEARTBEAT_STALE_MULT = 3                           # ts が interval×N 超なら「死亡」判定（読み手の目安）
